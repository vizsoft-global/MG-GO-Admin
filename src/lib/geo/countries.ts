/** ISO 3166-1 alpha-2 codes with English display names (admin UI). */
...
export function getCountryLabel(code: string | null | undefined): string | null {
  if (!code) return null;
  return countryByCode.get(code.toUpperCase())?.name ?? null;
}
...
export function normalizeCountryCode(raw: string | null | undefined): string | null {
  ...
  if (!COUNTRY_CODE_SET.has(trimmed)) return null;
  return trimmed;
}
