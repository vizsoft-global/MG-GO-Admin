...
/** Wider than a team column: the header carries team and criterion both. */
const CRITERION_COLUMN_WIDTH = 20;

/**
 * Header order, with one column per score component, then one per rating team,
 * then one per criterion. All three are tenant-configurable, so all three go
 * after the fixed columns — the fixed ones keep their positions whatever a
 * tenant has set up. Criteria go last because they are the finest cut: a reader
 * scanning team scores should not have to step over them.
 */
export function performanceReportHeaders(
  ...
  return [
    ...
    ...criteria.map((c) => `${c.team_label_en} · ${c.label_en} (1-5)`),
  ];
}
...
export function performanceReportRow(
  ...
  return [
    ...
    ...criteria.map((c) => {
      const score = row.manual_criteria[`${c.team_key}.${c.key}`];
      return score == null ? "—" : Math.round(score * 10) / 10;
    }),
  ];
}
...
export async function buildPerformanceReportXlsx(
  ...
  sheet.columns = [
    ...
    ...criteria.map(() => CRITERION_COLUMN_WIDTH),
  ].map((width) => ({ width }));

  const headerRow = sheet.addRow(
    performanceReportHeaders(teams, components, criteria),
  );
  headerRow.height = 22;
  ...
  for (const row of report.rows) {
    const dataRow = sheet.addRow(
      performanceReportRow(row, teams, components, criteria),
    );
