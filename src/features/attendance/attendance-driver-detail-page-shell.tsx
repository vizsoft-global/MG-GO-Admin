"use client";

import { useMemo } from "react";
import { useSearchParams } from "next/navigation";
import { useTranslations } from "next-intl";
import { Link, useRouter } from "@/i18n/navigation";
import { ArrowLeft, Loader2 } from "lucide-react";
import { AppPage, AppPageHeader, AppEmptyState } from "@/components/app";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { StatusPill } from "@/components/dashboard/status-pill";
import {
  LIVE_STATUS_LABEL_KEYS,
  addDays,
  formatDateTimeKuwait,
  formatDurationSeconds,
  kuwaitToday,
} from "./attendance-list-utils";
import {
  useDriverAttendanceDetail,
  useDriverAttendanceRange,
} from "./use-attendance-table";
import { fetchDriverAttendanceTimeline } from "./attendance-reporting-actions";
import { useQuery } from "@tanstack/react-query";
import { queryKeys } from "@/lib/query/query-keys";
import { resolveStatusVariant } from "@/lib/ui/resolve-status-variant";

export function AttendanceDriverDetailPageShell({ driverId }: { driverId: string }) {
  const t = useTranslations("pages.attendance");
  const router = useRouter();
  const searchParams = useSearchParams();
  const today = kuwaitToday();
  const date = searchParams.get("date") ?? today;
  const fromDate = addDays(date, -6);

  const { data: dayRow, isLoading: dayLoading } = useDriverAttendanceDetail(driverId, date);
  const { data: weekRows, isLoading: weekLoading } = useDriverAttendanceRange(
    driverId,
    fromDate,
    date,
  );

  const { data: timeline, isLoading: timelineLoading } = useQuery({
    queryKey: [...queryKeys.attendance.driverDetail(driverId, date, date), "timeline"],
    queryFn: () => fetchDriverAttendanceTimeline(driverId, date),
    enabled: Boolean(driverId),
  });

  const weekStats = useMemo(() => {
    if (!weekRows?.length) return { present: 0, late: 0, avgCompliance: 0 };
    const present = weekRows.filter((r) => r.check_in_at).length;
    const late = weekRows.filter((r) => r.minutes_late > 0).length;
    const scores = weekRows
      .map((r) => r.compliance_score)
      .filter((s): s is number => s != null);
    const avgCompliance =
      scores.length > 0
        ? Math.round(scores.reduce((a, b) => a + b, 0) / scores.length)
        : 0;
    return { present, late, avgCompliance };
  }, [weekRows]);

  const isLoading = dayLoading || weekLoading;

  return (
    <AppPage>
      <AppPageHeader
        title={dayRow?.driver_name ?? t("driverDetailTitle")}
        description={dayRow ? `${dayRow.driver_code} · ${date}` : date}
        actions={
          <Button variant="outline" size="sm" render={
            <Link href="/attendance">
              <ArrowLeft className="h-4 w-4" />
              {t("backToAttendance")}
            </Link>
          } />
        }
      />

      {isLoading ? (
        <div className="flex justify-center py-16">
          <Loader2 className="h-6 w-6 animate-spin text-muted-foreground" />
        </div>
      ) : !dayRow ? (
        <AppEmptyState title={t("emptyDriverDay")} description={t("emptyDriverDayHint")} />
      ) : (
        <div className="space-y-6">
          <div className="flex flex-wrap items-center gap-2">
            <StatusPill variant={resolveStatusVariant(dayRow.live_status)}>
              {t(LIVE_STATUS_LABEL_KEYS[dayRow.live_status] ?? "livePresent")}
            </StatusPill>
            <Input
              type="date"
              value={date}
              onChange={(e) => {
                const next = e.target.value;
                if (next) router.replace(`/attendance/drivers/${driverId}?date=${next}`);
              }}
              className="w-[160px]"
            />
          </div>

          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <div className="rounded-lg border border-border p-4">
              <p className="text-xs text-muted-foreground">{t("weekPresent")}</p>
              <p className="text-2xl font-semibold">{weekStats.present}</p>
            </div>
            <div className="rounded-lg border border-border p-4">
              <p className="text-xs text-muted-foreground">{t("kpiLate")}</p>
              <p className="text-2xl font-semibold">{weekStats.late}</p>
            </div>
            <div className="rounded-lg border border-border p-4">
              <p className="text-xs text-muted-foreground">{t("kpiCompliance")}</p>
              <p className="text-2xl font-semibold">{weekStats.avgCompliance}%</p>
            </div>
            <div className="rounded-lg border border-border p-4">
              <p className="text-xs text-muted-foreground">{t("colDuty")}</p>
              <p className="text-2xl font-semibold">
                {formatDurationSeconds(dayRow.duty_seconds)}
              </p>
            </div>
          </div>

          <div className="rounded-lg border border-border p-4">
            <h3 className="mb-3 text-sm font-semibold">{t("timelineTitle")}</h3>
            {timelineLoading ? (
              <Loader2 className="h-5 w-5 animate-spin text-muted-foreground" />
            ) : !timeline?.length ? (
              <p className="text-sm text-muted-foreground">{t("emptyTimeline")}</p>
            ) : (
              <ul className="space-y-2">
                {timeline.map((ev) => (
                  <li
                    key={`${ev.at}-${ev.kind}`}
                    className="flex items-center justify-between gap-4 text-sm"
                  >
                    <span>{ev.label}</span>
                    <span className="text-muted-foreground tabular-nums">
                      {formatDateTimeKuwait(ev.at)}
                    </span>
                  </li>
                ))}
              </ul>
            )}
          </div>
        </div>
      )}
    </AppPage>
  );
}
