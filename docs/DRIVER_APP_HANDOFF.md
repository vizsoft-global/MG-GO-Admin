...
*Last synced: 2026-08-19 — [admin+app] The 45-minute out-of-zone Home timer uses the assigned zone (0 m buffer), not restaurant `zone_status` from pickup heartbeats. Live Tracking V2 Live Events synthesises `gps.offline` when status is GPS Offline so the top row cannot stay on leftover "Entered zone". No schema or payload change — ship with the next Play build.*
