...
| **Live category screenshot flag** | `20260930100000` — both list and get return `screenshot_restricted` as `COALESCE(esign_categories.screenshot_restricted, esign_requests.screenshot_restricted)`. The request row still stores the create-time stamp; the rider must honour the **live category** value so flipping Screenshots to Allowed (or Blocked) applies to documents already sent. Missing key → fail closed (`true`). Open Full Document stays in-app; do not hand the signed URL to Notes / Downloads (that backgrounds the app, trips the content-hidden overlay, and shows the system “Starting download” sheet). |
