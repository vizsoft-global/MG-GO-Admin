...
**Live Tracking V2 edge rail (additive — V1 above is unchanged):** when `LIVE_INGEST_URL` is set, the duty FGS also streams fixes to a Cloudflare Worker at `POST {LIVE_INGEST_URL}/ingest` (driver Supabase JWT as bearer, batches of ≤3), at a **fixed 5s** while moving and **30s** while idle. The jitter that used to spread reports over 10–15s is gone because the admin renderer interpolates between fixes and cannot do that against an unknown next-arrival time. `driver_report_location` remains the durable record and the safety net: the Durable Object flushes to `driver_locations` every 10s, so the app skips its own durable write for **25s** after a successful publish, and writes immediately if a publish fails. State changes, the first sample and `delivery_submit` always write directly, never waiting on a flush.
...
*Last synced: 2026-08-14 — [admin+app] Live Tracking V2 edge rail: fixed 5s moving / 30s idle cadence, `POST {LIVE_INGEST_URL}/ingest`, `replay` flag on queued fixes, `duty_state_version` guard. `driver_report_location` unchanged and still the durable record. Migrations `20260914100000`–`20260914100200`.*
