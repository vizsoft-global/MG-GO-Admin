...
**Arabic overflow is the only part that still needs a real run**, because line-breaking is font-specific and the app resolves its typeface through `google_fonts` at runtime. A Pixel 9 emulator is available, so the device is not the blocker — the blocker is that `Env` hard-asserts the driver app may only point at the production Supabase project. Reaching an RCM screen therefore means signing in as a live rider, and both riders in the QA seed carry consequences: `10002` has 1,098 deliveries and an active device session that a login would end, and opening a seeded document as `10001` would permanently stamp `viewed_at` through the first-open-wins `driver_mark_esign_viewed`. Left as a decision rather than taken unilaterally.
