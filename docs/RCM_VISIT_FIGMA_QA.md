...
**RSup/27 e-sign viewer — most of it followed.** The preview is the only part of that screen that needs a network: an image goes through `Image.network`, a PDF is handed to `url_launcher`. Everything around it is plain layout, and that is where Arabic overflows. Two branches are now covered at Pixel 9 in Arabic — *no document attached*, which also renders the amber screenshot-restriction banner at its longer Arabic length and both footer buttons (`رفض` / `توقيع المستند`) inside the viewport, and *preview failed*, which the test gets for free: `Supabase.instance` is not initialised under `flutter test`, `_loadDocument` catches that, and the screen must land on `تعذّر تحميل معاينة المستند.` with a retry button rather than a stuck spinner. The row stays PARTIAL for the resolved preview only.
