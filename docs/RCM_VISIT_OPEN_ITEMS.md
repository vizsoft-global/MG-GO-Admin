...
- **One cosmetic finding.** A long Arabic label on a *custom* request type wraps to two lines and makes its hub tile taller than the others in its row. Built-in labels are all short enough that this never shows; it only appears for types an admin creates.
