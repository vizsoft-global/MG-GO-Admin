...
1. ~~Workflow SLA and breach-action columns~~ — ✅ Shipped 2026-08-12. `request_approval_step_templates.sla_minutes` / `breach_action`, mirrored onto each live step as `sla_due_at` / `breach_action` / `sla_breached_at` and onto `requests.sla_due_at`. An hourly sweep (`admin_run_request_sla_sweep`) marks the breach and raises the attention badge. **Every SLA is NULL until an admin sets one** — no duration was invented, so the sweep is inert until the workflow builder is used. Breach action never decides a step: `notify` and `escalate` differ only in `attention_reason`, because auto-advancing a step is auto-approving it by another name. **Answered: `escalate` stays a louder notification and does not re-route the step.** Re-routing would hand the decision to someone the workflow did not name, and the workflow builder already covers that case properly — remove the step, or give it a different role.
