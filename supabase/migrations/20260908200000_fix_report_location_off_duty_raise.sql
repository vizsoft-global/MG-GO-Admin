-- Off-duty location reports must raise driver_off_duty, not a parser error.
--
-- `RAISE EXCEPTION 'code' USING MESSAGE = 'text'` gives the message twice, so
-- PL/pgSQL aborts with "RAISE option already specified: MESSAGE" before the
-- condition can propagate. 20260711100000 removed this pattern across the
-- driver RPCs; 20260822100000 reintroduced it in driver_report_location and
-- 20260908170000 carried it forward. The driver app matches on the code, so an
-- off-duty background report surfaced as an unmapped Postgres error instead of
-- "you are off duty". Context moves to DETAIL, which does not collide.
