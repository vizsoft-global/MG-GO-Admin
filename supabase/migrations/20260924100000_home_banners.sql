-- Admin-managed Home banner: one image + caption, targeted like notifications.
-- Resolution rides inside driver_get_home_dashboard so the app already caches it offline.

CREATE TABLE IF NOT EXISTS public.driver_home_banners (
  ...
  driver_group_ids uuid[] NOT NULL DEFAULT '{}',
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  created_by uuid REFERENCES public.profiles(id),
  CONSTRAINT driver_home_banners_date_range_chk
    CHECK (starts_at IS NULL OR ends_at IS NULL OR ends_at >= starts_at)
);

COMMENT ON TABLE public.driver_home_banners IS
  'Promotional Home banner. Empty targeting arrays mean every driver. Images live in the public branding bucket at home-banners/*.';

CREATE INDEX IF NOT EXISTS driver_home_banners_active_sort_idx
  ON public.driver_home_banners (is_active, sort_order, created_at DESC);

ALTER TABLE public.driver_home_banners ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS driver_home_banners_staff_all ON public.driver_home_banners;
CREATE POLICY driver_home_banners_staff_all ON public.driver_home_banners
  FOR ALL TO authenticated
  USING (public.is_admin_panel_user())
  WITH CHECK (public.is_admin_panel_user());

CREATE OR REPLACE FUNCTION public._driver_home_banner_for(p_driver_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
DECLARE
  ...
  v_row public.driver_home_banners%ROWTYPE;
BEGIN
  SELECT d.zone_id, d.partner_id
  INTO v_zone, v_partner
  FROM public.drivers d
  WHERE d.id = p_driver_id;

  SELECT b.*
  INTO v_row
  FROM public.driver_home_banners b
  WHERE b.is_active
    AND (b.starts_at IS NULL OR b.starts_at <= v_now)
    AND (b.ends_at IS NULL OR b.ends_at >= v_now)
    AND (cardinality(b.zone_ids) = 0 OR v_zone = ANY (b.zone_ids))
    AND (cardinality(b.partner_ids) = 0 OR v_partner = ANY (b.partner_ids))
    AND (
      cardinality(b.driver_group_ids) = 0
      OR EXISTS (
        SELECT 1
        FROM public.driver_group_members m
        WHERE m.driver_id = p_driver_id
          AND m.group_id = ANY (b.driver_group_ids)
      )
    )
  ...
  RETURN jsonb_build_object(
    ...
    'caption_en', v_row.caption_en,
    'caption_ar', v_row.caption_ar,
    'deep_link', v_row.deep_link
  );
END;
$$;

REVOKE ALL ON FUNCTION public._driver_home_banner_for(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public._driver_home_banner_for(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public._driver_home_banner_for(uuid) TO service_role;
