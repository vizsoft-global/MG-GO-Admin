-- Delivery Orders Report: join-based shift attribution.
-- The 20260818 version scanned every shift window once per delivery (and the
-- planner evaluated that COALESCE three times for BETWEEN). Year-to-date
-- (~38k deliveries) ran ~25s and hit authenticated statement_timeout=8s.
...
COMMENT ON FUNCTION public.report_delivery_orders(date, date) IS
  'Per-driver per-shift-day delivery counts for the Orders Report matrix export. Attributes deliveries to submitted shift windows via driver-scoped joins; falls back to nearest shift or Kuwait calendar day. Max 366 inclusive days.';
