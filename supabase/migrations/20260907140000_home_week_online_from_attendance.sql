-- This Week's Progress Time in was summing driver_sessions, so a leftover
-- is_online row from last week filled the current week with wall-clock hours.
-- Use per-day attendance.online_seconds (plus today's open stretch only).

CREATE OR REPLACE FUNCTION public.driver_week_online_seconds(
  p_driver_id uuid,
  p_week_start date,
  p_today date
)
...
DECLARE
  ...
  v_live bigint := 0;
BEGIN
  SELECT COALESCE(SUM(da.online_seconds), 0)::bigint
  INTO v_seconds
  FROM public.driver_attendance da
  WHERE da.driver_id = p_driver_id
    AND da.attendance_date BETWEEN p_week_start AND p_today;

  SELECT GREATEST(
    0,
    EXTRACT(EPOCH FROM (now() - ds.went_online_at))::bigint
  )
  INTO v_live
  ...
  WHERE ds.driver_id = p_driver_id
    ...
    AND (ds.went_online_at AT TIME ZONE 'Asia/Kuwait')::date = p_today
  ORDER BY ds.went_online_at DESC
  LIMIT 1;

  RETURN v_seconds + COALESCE(v_live, 0);
END;
$$;

REVOKE ALL ON FUNCTION public.driver_week_online_seconds(uuid, date, date) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.driver_week_online_seconds(uuid, date, date) TO authenticated;
GRANT EXECUTE ON FUNCTION public.driver_week_online_seconds(uuid, date, date) TO service_role;
...
COMMENT ON FUNCTION public.driver_week_online_seconds(uuid, date, date) IS
  'Sum driver_attendance.online_seconds for [week_start, today] plus live elapsed of an is_online session that started today. Stale sessions from prior days are ignored.';
