-- Payroll & Requests hub: one-month snapshot + payroll.view / payroll.export.
-- Formulas match src/features/payroll/payroll-formulas.ts. Do not treat this as
-- Performance's orders-based working day.

INSERT INTO public.admin_permissions (slug, label, category) VALUES
  ('payroll.view', 'View payroll & requests', 'payroll'),
  ('payroll.export', 'Export payroll & requests', 'payroll')
ON CONFLICT (slug) DO UPDATE SET
  label = EXCLUDED.label,
  category = EXCLUDED.category;

INSERT INTO public.admin_role_permissions (role_id, permission_slug)
SELECT a.role_id, 'payroll.view'
FROM public.admin_role_permissions a
WHERE a.permission_slug = 'attendance.view'
  AND EXISTS (
    SELECT 1
    FROM public.admin_role_permissions r
    WHERE r.role_id = a.role_id
      AND r.permission_slug = 'requests.view'
  )
ON CONFLICT DO NOTHING;

INSERT INTO public.admin_role_permissions (role_id, permission_slug)
SELECT a.role_id, 'payroll.export'
FROM public.admin_role_permissions a
WHERE a.permission_slug = 'attendance.view'
  AND EXISTS (
    SELECT 1
