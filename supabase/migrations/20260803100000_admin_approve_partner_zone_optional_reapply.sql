-- Re-apply admin_approve_driver approval rules for environments that still run the
-- pre-20260801 version requiring partner_id and zone_id on driver_intakes.
-- Create/edit intake treats partner and zone as optional (20260615100000).
...
COMMENT ON FUNCTION public.admin_approve_driver(uuid, uuid, text) IS
  'Staff-only: create profile + driver from intake, activate, mint passcode. Requires identity fields + at least one active restaurant; partner and zone are optional.';
