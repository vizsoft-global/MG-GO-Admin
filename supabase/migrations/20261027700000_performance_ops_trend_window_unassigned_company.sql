-- Trend window + Unassigned company. DROP first so the extra args are not an overload.

DROP FUNCTION IF EXISTS public.admin_performance_ops_snapshot(date, date, text[], uuid[], text[], text[], text[], text[], uuid[], boolean, text);
DROP FUNCTION IF EXISTS public.admin_performance_ops_snapshot(date, date, text[], uuid[], text[], text[], text[], text[], uuid[], boolean, text, date, date);

CREATE FUNCTION public.admin_performance_ops_snapshot(
  p_from date,
  p_to date,
...
END;
$$;
REVOKE ALL ON FUNCTION public.admin_performance_ops_snapshot(date, date, text[], uuid[], text[], text[], text[], text[], uuid[], boolean, text, date, date) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_performance_ops_snapshot(date, date, text[], uuid[], text[], text[], text[], text[], uuid[], boolean, text, date, date) TO authenticated;
