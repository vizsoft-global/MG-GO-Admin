-- Earnings Performance Summary was summing driver_earnings_daily.deliveries
-- (verified-only, and only days with an earnings row). Expose lifetime
-- submitted vs verified counts for the driver app.

CREATE OR REPLACE FUNCTION public.driver_delivery_performance_counts(p_driver_id uuid)
RETURNS jsonb
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT jsonb_build_object(
    'total_deliveries', count(*) FILTER (
      WHERE status IS DISTINCT FROM 'cancelled'::public.delivery_status
    )::int,
    'verified_deliveries', count(*) FILTER (
      WHERE status = 'verified'::public.delivery_status
    )::int,
    ...
    'rejected_deliveries', count(*) FILTER (
      WHERE status = 'rejected'::public.delivery_status
    )::int
  )
...
CREATE OR REPLACE FUNCTION public.driver_get_earnings_summary()
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  RETURN public.driver_delivery_performance_counts(v_uid) || jsonb_build_object('ok', true);
END;
$$;

REVOKE ALL ON FUNCTION public.driver_delivery_performance_counts(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.driver_get_earnings_summary() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.driver_delivery_performance_counts(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.driver_get_earnings_summary() TO authenticated;
GRANT EXECUTE ON FUNCTION public.driver_delivery_performance_counts(uuid) TO service_role;
GRANT EXECUTE ON FUNCTION public.driver_get_earnings_summary() TO service_role;

COMMENT ON FUNCTION public.driver_get_earnings_summary() IS
  'Rider: lifetime delivery counts for Earnings Performance Summary. total_deliveries excludes cancelled; verified_deliveries is payroll-verified only.';
