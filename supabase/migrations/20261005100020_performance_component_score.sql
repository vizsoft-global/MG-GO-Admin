-- The compliance pillar becomes a weighted blend of named components.
--
-- Three things change in admin_list_driver_performance, and each has a wrong
-- answer that looks reasonable:
--
-- 1. compliance_score is the component blend rather than
--    AVG(v_attendance_daily.compliance_score). That old number is a flat 70 for
--    any lateness, so one minute and ninety minutes score identically — it
--    cannot rank the thing it measures.
--
-- 2. The exception penalty stops applying to LateCheckIn, EarlyLogout,
--    OutsideZone and MissingLocationUpdates. Those four are now components, and
--    subtracting them again would charge a driver twice for one fact. NoCheckIn,
--    NoAssignedShift and OfflineDuringShift keep their penalty: they are absence
--    and roster gaps, which no component measures.
--
-- 3. A null pillar is dropped along with its weight, exactly as the manual term
--    already is. Before this, a driver with nothing measurable would have scored
--    0 on the pillar — which reads as "measured, and terrible" rather than "not
--    measured". Renormalising instead gives the score they would have had with
--    that pillar not configured at all.
--
-- Component values come from driver_performance_daily for closed days and from
-- performance_daily_source for today, so the period tab and the live tab keep
-- agreeing about the same driver on the same day.
