-- Optional driver nationality (ISO 3166-1 alpha-2) on intakes and linked drivers.
...
ALTER TABLE public.driver_intakes
  DROP CONSTRAINT IF EXISTS driver_intakes_nationality_format_chk;

ALTER TABLE public.drivers
  DROP CONSTRAINT IF EXISTS drivers_nationality_format_chk;

ALTER TABLE public.driver_intakes
  ADD CONSTRAINT driver_intakes_nationality_format_chk
  CHECK (nationality IS NULL OR nationality ~ '^[A-Z]{2}$');

ALTER TABLE public.drivers
  ADD CONSTRAINT drivers_nationality_format_chk
  CHECK (nationality IS NULL OR nationality ~ '^[A-Z]{2}$');

COMMENT ON COLUMN public.driver_intakes.nationality IS
  'Optional ISO 3166-1 alpha-2 nationality code (e.g. KW, IN).';

COMMENT ON COLUMN public.drivers.nationality IS
  'Optional ISO 3166-1 alpha-2 nationality code; copied from intake on approve.';
