-- Rider profiles inherit approval_status default 'pending', which made them appear in
-- admin "Access requests" widgets. Staff panel access uses role=staff; riders should
-- not remain in pending admin-approval state after Verify & approve.
...
WHERE role = 'rider'::public.app_role
  AND approval_status = 'pending'::public.admin_approval_status;
...
COMMENT ON FUNCTION public.admin_approve_driver(uuid, uuid, text) IS
  'Staff-only: create profile + driver from intake, activate, mint passcode. Rider profiles are marked approval_status=approved (not staff access requests).';
